...
route.post('/login', async (req, res) => {
  try {
    ...
    if (
      !validaString(account, 1, fieldLenght.email) ||
      !validaString(password, 1, fieldLenght.id, 1)
    ) {
      paramErr(res, req);
      return;
    ...
    if (loginVerifyLimit.verify(ip, account)) {
      const userinfo = (
        await queryData(
          'user',
          'verify,account,username,password',
          `WHERE (account = ? OR username = ? OR email = ?) AND state = ? AND account != ?`,
          [account, account, account, 1, 'hello']
        )
      )[0];
...
route.post('/verify-login', async (req, res) => {
  try {
    const { token, account: acc, password } = req.body;
...
route.get('/mail-code', async (req, res) => {
  try {
    ...
    if (!validaString(acc, 1, fieldLenght.email)) {
      paramErr(res, req);
      return;
    }
