...
import { jwtde } from '../../utils/jwt.js';
...
route.get('/', async (req, res) => {
  try {
    ...
    const jwtData = token ? jwtde(token).data : '';

    // 验证外部播放器临时访问权限
    if (!account && jwtData && jwtData.type === 'temAccessFile') {
    ...
    if (dir === 'upload') {
      ...
      if (account) {
        path = _path.normalize(
          `${configObj.filepath}/music/${pArr.slice(1).join('/')}`
        );
      } else {
        const sid = pArr[0];
      ...
      if (
        ...
        if (dir === 'file') {
          thumbP = _path.normalize(
            `${configObj.filepath}/thumb/${dir}/${_path.basename(path)[1]}_${
              stat.size
            }.png`
          );
        } else {
          thumbP = _path.normalize(
            `${configObj.filepath}/thumb/${
              _path.extname(path.slice(configObj.filepath.length))[0]
            }.png`
          );
        }
