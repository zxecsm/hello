...
// 设置当前开启的歌单id
let curOpenSongListId = '';
export function setCurOpenSongListId(val) {
  if (val === undefined) {
    return curOpenSongListId;
  }
  curOpenSongListId = val;
}
// 返回按钮
...
export function moveSongToList(e, pid, ar) {
  ...
  const html = _tpl(
    ...
    <div v-for="v in list" cursor="y" class="item" :data-id="v.id">
      <img style="width: 4rem;height: 4rem;border-radius: var(--border-radius1);" :data-src="v.pic"/>
    ...
    if (mouseSongListFromDom) {
      ...
      if (to) {
        const fIdx = $(mouseSongListFromDom).index(),
          tIdx = $(to).index(),
          fromId = mouseSongListFromDom.dataset.id,
          toId = to.dataset.id;
...
function renderSongList() {
  ...
  const html = _tpl(
    `
    <li v-for="item in arr" class="song_list_item" :data-id="item.id" cursor="y">
      <div class="list_logo">
...
async function renderSongs(gao) {
  songsBoxSelector.stop();
  const listId = curOpenSongListId;
  if (!listId) return;
...
longPress($songListWrap[0], '.song_list_item', function (e) {
  if (songListMouseElementTracker.active) return;
  const id = $(this).attr('data-id');
...
function getSongInfo(id) {
  const p = musicList.find((item) => item.id === curOpenSongListId);
...
function isSelectingSongs() {
  const $checkMenu = $msuicContentBox.find('.check_all_menu_wrap');
  if ($checkMenu.length === 0) return false;
...
function stopSelectSongs() {
  $msuicContentBox.find('.check_all_menu_wrap').css('display', 'none');
  $msuicContentBox
    .find('.list_items_wrap .check_state')
    .css('display', 'none')
    .attr('check', 'n')
...
function songMenu(e, idx, sobj) {
  ...
  if (idx > 0 && curOpenSongListId != 'all' && curSongListSort === 'default') {
    data.unshift({
      id: '14',
      text: '置顶',
...
$msuicContentBox
  ...
  .on('contextmenu', '.song_item', function (e) {
    e.preventDefault();
    if (isMobile() || isSelectingSongs()) return;
    startSelectSongs();
  ...
  $msuicContentBox.find('.list_items_wrap')[0],
  {
    ...
    onStart({ e }) {
      ...
      if (
        ...
        return true;
      mouseSongsFromDom = item;
      const obj = getSongInfo(item.dataset.id);
      songsMouseElementTracker.changeInfo(`${obj.artist} - ${obj.title}`);
    },
    onMove() {
      allowSlide.update();
    },
...
export function highlightPlayingSong(isPosition) {
  ...
  if (temPlaylist != undefined) {
    if (
      (curOpenSongListId === 'all' && temPlaylist.length > 0) ||
...
// 显示播放器
let showMusicPlayerBoxFlag = false;
let showMusicPlayerBoxOnce = false;
export function showMusicPlayerBox(cb) {
  hideRightMenu();
  if (!showMusicPlayerBoxFlag) {
    createAutoHide(
      10,
      '.music_lrc_wrap',
      '.lrc_foot_wrap',
  ...
  lrcScroll(true);
  if (!showMusicPlayerBoxOnce) {
    showMusicPlayerBoxOnce = true;
...
_mySlide({
  el: '.list_items_wrap',
  right() {
    if (!isSelectingSongs() && allowSlide.allow()) {
      musicBackBtn();
    }
  },
});
