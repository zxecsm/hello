...
function renderList() {
  ...
  const html = _tpl(
    `
    <li v-for="{title,des,id} in list" :data-id="id" cursor="y" class="bm_item jzxz">
      <div class="logo"></div>
  ...
  reqBmkGetShare({ id: HASH, pass: passCode })
    .then((res) => {
      if (res.code === 1) {
        ...
        let { username, logo, account, data, title, exp_time, email } =
          res.data;
...
function saveBm(e) {
  rMenu.inpMenu(
    ...
    debounce(
      function ({ close, inp }) {
        reqBmkSaveShare({ id: HASH, title: inp.title, pass: passCode })
          .then((res) => {
            if (res.code === 1) {
