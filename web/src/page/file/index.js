...
async function renderList(top) {
  ...
  if (paging.list.length > 0) {
    paging.list.forEach((item) => {
      ...
      str += `<ul draggable="true" class="file_item" data-id="${id}">
      <li class="check_state" check="n"></li>
      <li cursor class="logo iconfont ${logo || 'is_img'}"></li>
      <li cursor class="name"><span class="text">${text}</span></li>
      <li ${type == 'file' ? '' : 'cursor'} class="size">${
        size ? computeSize(size) : type == 'file' ? '--' : '计算'
      }</li>
...
function renderFoot() {
  ...
  if (len > 0) {
    ...
    if (isRoot()) {
      str += `<button cursor class="f_fdelete btn btn_danger">强制删除</button>`;
    }
...
function showPaste() {
  $header
    .find('.paste_btn')
    .html(
      `<span cursor class="text">粘贴(${waitObj.data.length})</span><span cursor class="close iconfont icon-guanbi"></span>`
    )
    .css('display', 'block');
}
function hidePaste() {
